...
class UserRegistrationForm(forms.ModelForm):
    ...
    def clean_username(self):
        ...
        if user.exists():
            raise forms.ValidationError("Користувач із таким іменем уже існує")
...
class UserProfileUpdateForm(forms.ModelForm):
    ...
    def clean_username(self):
        username = self.cleaned_data.get('username')
        model = self.Meta.model
        user = model.objects.filter(username__iexact=username).exclude(
            pk=self.instance.pk)
    ...
    def clean_email(self):
        email = self.cleaned_data.get('email')
        model = self.Meta.model
        user = model.objects.filter(email__iexact=email).exclude(
            pk=self.instance.pk)
